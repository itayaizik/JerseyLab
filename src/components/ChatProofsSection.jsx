...
export default function ChatProofsSection({ title }) {
  ...
  const subtitle = proofs.length && reviews.length
    ? 'צילומי שיחות מהוואטסאפ ותמונות ששלחו לקוחות. לחצו להגדלה.'
    : proofs.length ? 'צילומי מסך מהוואטסאפ. לחצו כדי לקרוא.' : 'תמונות ששלחו לקוחות. לחצו להגדלה.';

  return (
    <section className="mt-16 sm:mt-24" aria-labelledby="chat-proofs-heading">
      <div className="shop-container">
        <SectionHeader id="chat-proofs-heading" title={title || 'לקוחות מספרים'} subtitle={subtitle} />
        ...
        <div className="mt-10">
          <ScrollRow label="לקוחות מספרים" itemClassName="w-[300px] sm:w-[360px]">
            {[
              ...proofs.map(proof => (
                <figure key={proof.id} className="h-full">
                  <button type="button" onClick={() => setLightbox({ image: proof.image_url, caption: proof.caption })} aria-label="הגדלת צילום השיחה"
              ...
              ...reviews.map(review => {
                const name = review.is_anonymous ? 'לקוח' : (review.reviewer_name || 'לקוח');
                return (
                  <figure key={`review-${review.id}`} className="flex h-full flex-col rounded-3xl bg-white p-2 shadow-card">
                    <button type="button" onClick={() => setLightbox({ image: review.image_url, caption: review.comment })} aria-label="הגדלת התמונה"
                      className="block w-full cursor-zoom-in">
                      <img src={review.image_url} alt={`תמונה ששלח ${name}`} loading="lazy"
                    ...
                    <figcaption className="flex flex-1 flex-col px-3 pb-3 pt-3.5">
                      <span className="flex gap-0.5" aria-label={`${review.rating} מתוך 5`}>
                        {[1, 2, 3, 4, 5].map(s => (
                          <Star key={s} className={`h-4 w-4 ${s <= review.rating ? 'fill-brand-orange text-brand-orange' : 'text-brand-line'}`} aria-hidden="true" />
                        ))}
                      </span>
                      {review.comment && (
                        <span className="mt-2 line-clamp-2 text-[14px] leading-snug text-brand-navy/80">"{review.comment}"</span>
                      )}
