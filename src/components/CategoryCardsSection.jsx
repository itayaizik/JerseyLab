...
// The cards' words are typed into the admin in Hebrew. The ones in use are
// translated here; anything else keeps its Hebrew on the English site, and a
// subtitle without a translation is left off rather than shown in Hebrew.
const CARD_WORDS = {
  'ילדים': 'Kids',
  'מידות ילדים': "Kids' sizes",
  'קלאסיקות נצחיות': 'Timeless classics',
  'קלאסיקות מהארכיון': 'Classics from the archive',
  'ליגות': 'Leagues',
  'אנגליה, ספרד ועוד': 'England, Spain and more',
  'אנגליה • ספרד • עוד': 'England • Spain • more',
  'מונדיאל 2026': 'World Cup 2026',
  'שחקנים': 'Players',
  'חולצות עם שם': 'Shirts with names',
  'כל הגלים ובחורות': 'All ages',
  'סייל ⚡': 'Sale ⚡',
  'סייל': 'Sale',
  'מחירים מטורפים': 'Great prices',
};

const cardText = (text) => (isEn && text ? (CARD_WORDS[text.trim()] || term(text)) : text);
const cardSubtitle = (text) => {
  if (!isEn || !text) return text;
  const english = CARD_WORDS[text.trim()] || term(text);
...
export default function CategoryCardsSection({ title }) {
  ...
  return (
    <section className="shop-container mt-16 sm:mt-24" aria-labelledby="categories-heading">
      <SectionHeader id="categories-heading" title={title || t('קנו לפי קטגוריה', 'Shop by category')} />
      <ul className={`mt-10 grid grid-cols-2 gap-3 sm:gap-4 ${columns}`}>
        {displayCats.map((cat, i) => {
          ...
          const subtitle = cardSubtitle(cat.subtitle);
          return (
              ...
              >
                ...
                <span className="absolute inset-x-2.5 bottom-2.5 flex items-center justify-between gap-2 rounded-2xl bg-white/90 px-3.5 py-3 backdrop-blur sm:inset-x-3 sm:bottom-3 sm:px-4">
                  <span className="min-w-0">
                    <span className="block truncate text-[15px] font-semibold text-brand-navy">{cardText(cat.label)}</span>
                    {subtitle && <span className="block truncate text-xs text-brand-navy/55">{subtitle}</span>}
                  </span>
